...
module Locomotive
  module Entities
    class Page < Entity
      ...
      field :editable_elements, type: :array, class_name: 'Locomotive::Entities::EditableElement'
      ...
      def model=(content_type)
        Locomotive::Common::Logger.warn 'The model attribute is deprecated. Use content_type instead.'
      ...
      def set_editable_elements(attributes)
        ...
        attributes.to_a.each do |_attributes|
          ...
          if editable_element = self.find_editable_element(_attributes['block'], _attributes['slug'])
            ...
            self.editable_elements << Locomotive::Entities::EditableElement.new(_attributes)
          end
        end
      end
      ...
      def localize_fullpath(locales = nil)
        ...
        locales.each do |locale|
          Locomotive::Models.with_locale(locale) do
      ...
      #
      def to_params
        params = self.filter_attributes %w(title parent_id slug redirect_url redirect_type handle
          listed published searchable cache_strategy response_type position templatized seo_title
